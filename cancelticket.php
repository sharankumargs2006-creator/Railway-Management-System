<?php 
    ...
    if(isset($_POST['cticket'])){
        ...
        $email = '';
        $user_name = '';
        $ticket_sql = "SELECT ti.email, ti.username, t.train_name FROM ticket ti 
                       JOIN station s ON ti.station_no = s.station_no 
                       JOIN train t ON s.train_no = t.train_no 
                       WHERE ti.ticket_no = '$pnr' LIMIT 1";
        ...
        if($ticket_result && $ticket_result->num_rows > 0){
            ...
            $user_name = $ticket_row['username'];
            $train_name = $ticket_row['train_name'];
        }

        // if ticket already cancelled then the query not reexecute on resubmit of page 
        if(isset($_SESSION['update'])){
            echo "<script> alert('ticket already cancelled'); </script>";
        }
        else{
            // instead of deleting data we used status col 
            //if ticket cancel it going to set cancelled
            ...
            if($conn->query($sql) == true){
                ...
                if (!empty($email)) {
                    ...
                    try {
                        ...
                        $mail->SMTPAuth   = true;
                        $mail->Username   = 'raghavendravmujumadar@gmail.com'; // Your Gmail address
                        $mail->Password   = 'lfdu sedz vcde bwhr';    // Your Gmail app password
                        $mail->SMTPSecure = PHPMailer\PHPMailer\PHPMailer::ENCRYPTION_STARTTLS;
                        $mail->Port       = 587;

                        //Recipients
                        $mail->setFrom('raghavendravmujumadar@gmail.com', 'Railway Reservation');
                        $mail->addAddress($email, $user_name);

                        // Content
                        $mail->isHTML(true);
                        $mail->Subject = 'Ticket Cancellation Confirmation';
                        $mail->Body    = "Dear $user_name,<br><br>Your ticket (PNR: <b>$pnr</b>) for train <b>$train_name</b> has been <b>cancelled</b> successfully.<br><br>Thank you for using our service.<br>Indian Railways";

                        $mail->send();
                        echo "<script>alert('Cancellation email sent!');</script>";
                    } catch (Exception $e) {
...
<!-- search button for train -->
    <div class="container">
        ...
        <div class="row">
            <div class="col-12">
                <h4 class="navbar-brand text-primary">Cancel ticket:</h4>
            </div>
            <div class="col-8">
