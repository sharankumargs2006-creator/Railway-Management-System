...
use PHPMailer\PHPMailer\Exception;
 ...
 /*   
// New block for Pay button
if(isset($_POST['pay'])) {
    // Capture payment form fields
    $cc_number = isset($_POST['cc_number']) ? $_POST['cc_number'] : '';
    $cc_exp = isset($_POST['cc_exp']) ? $_POST['cc_exp'] : '';
    $cvv = isset($_POST['cvv']) ? $_POST['cvv'] : '';
    $nameoncard = isset($_POST['nameoncard']) ? $_POST['nameoncard'] : '';

    // Retrieve booking details from session
    if(isset($_SESSION['booking_details'])) {
        extract($_SESSION['booking_details']);
    } else {
        echo "<script>alert('Booking details missing. Please start over.'); window.location='./index.php';</script>";
        exit;
    }
}
    */
...
if(isset($_POST['pay'])){        
       ...
       /* 
        if(isset($_POST['pay']) && payvalid()) {
    // PHPMailer code here
    $mail = new PHPMailer(true);
    try {
        // Existing PHPMailer configuration
        $mail->send();
        include('send_mail.php');
        echo "<script>alert('Payment successful and email sent!'); window.location='./index.php';</script>";
    } catch (Exception $e) {
        error_log("Mail Error: {$mail->ErrorInfo}");
        echo "<script>alert('Payment processed, but email failed to send.');</script>";
    }
}*/
...
try {
    ...
    $mail->SMTPAuth = true;
    $mail->Username = 'raghavendravmujumadar@gmail.com';  // Your email
    $mail->Password = 'vytf iivj iycl jfte';     // Use App Password if using Gmail
    $mail->SMTPSecure = 'tls';
    $mail->Port = 587;

    // Recipients
    $mail->setFrom('raghavendravmujumadar@gmail.com', 'Indian Railways');
    $mail->addAddress($email); // Customer email from form
    ...
    $mail->Body    = "
        <h3>Dear $uname,</h3>
        <p>Your ticket from <strong>$src</strong> to <strong>$dest</strong> has been successfully booked.</p>
        <ul>
            <li>Train: $train_name ($train_no)</li>
            <li>Date: $date</li>
            <li>Departure: $dep_time</li>
            <li>Arrival: $arr_time</li>
            <li>Class: $class</li>
            <li>PNR: $pnr</li>
        </ul>
        <p>Thank you for using Indian Railways Booking System.</p>
    ";

    $mail->send();
    // echo "Message sent successfully";
    // After saving booking info to DB (make sure $user['email'] and $user['username'] are set)
 include('send_mail.php');  // Moved after payment success

// Example: Get user data from session or DB
//$userEmail = $_SESSION['email'];    // Or fetch from DB if not in session
//$userName = $_SESSION['uname'];     // Replace based on your session variables

// You can dynamically generate ticket info here
//$ticketInfo = "Train: $trainNo<br>From: $from<br>To: $to<br>Date: $date<br>PNR: $pnr";

// Call the function to send email
//sendBookingEmail($userEmail, $userName, $ticketInfo);
