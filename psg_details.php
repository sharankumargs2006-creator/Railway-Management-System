    ...
    <div class="row">
        ...
        <div class="col-9">
            <form action="./review.php" name="psgForm" method="post" onsubmit="return(check());">
                <!-- input block -->
                <?php for($i = 1; $i <= 5; $i++) { ?>
                <div class="row alert alert-dark shadow p-2 pb-3 bg-white" id="<?php echo $i; ?>" style="display: none;">
                    <div class="col-12 mb-3">
                        <img src="asset/img/logo/cardG.png" width="25">
                        <span>&nbsp;<?php echo $i; ?></span>
                        <?php if($i > 1) { ?>
                            <button type="button" onclick="remove(<?php echo $i; ?>)" class="close">
                    ...
                    <div class="row w-100 mx-0">
                        <div class="col-md-4">
                            <div class="form-group mb-0">
                                <input class="form-control hvr-shadow" 
                                       type="text" 
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-0">
                                <select name="gender<?php echo $i; ?>" 
                                        class="custom-select hvr-shadow"
                                        <?php echo $i == 1 ? 'required' : ''; ?>>
        ...
        <div class="col-12 col-sm-3 pl-4">
            <div class="card shadow-cust">
                ...
                <div class="card-body pt-0 pb-0 text-center">
                   ...
                   <div class="alert-primary p-3 rounded">
            <div class="journey-details">
                <!-- Departure -->
                <div class="departure mb-3">
                    <h6 class="text-muted mb-1"><?php echo date('l, d M Y', strtotime($date)); ?></h6>
                    <h5 class="font-weight-bold mb-1"><?php echo htmlspecialchars($src); ?></h5>
                    <small class="text-muted">Departure: <?php echo date('h:i A', strtotime($dep_time)); ?></small>
                </div>
                ...
                <!-- Arrival -->
                <div class="arrival mt-3">
                    <h5 class="font-weight-bold mb-1"><?php echo htmlspecialchars($dest); ?></h5>
                    <small class="text-muted">Arrival: <?php echo date('h:i A', strtotime($arr_time)); ?></small>
                </div>
            </div>
        </div>
                </div>
...
if(empty($selected_seats)) {
    echo "<script>alert('Please select seats first'); window.location.href='train_list.php';</script>";
    exit();
}
?>
<script type="text/javascript">
    // Hide all passenger forms initially
    document.addEventListener('DOMContentLoaded', function() {
        // Show only the number of forms needed for selected seats
        let totalSeats = <?php echo $total_seats; ?>;
        for(let i = 1; i <= 5; i++) {
            let form = document.getElementById(i);
            if(form) {
                form.style.display = i <= totalSeats ? 'block' : 'none';
            }
        }
    });
</script>
...
if(empty($train_no) || empty($selected_seats)) {
    echo "<script>
        alert('Please complete your seat selection');
        window.location.href = 'train_list.php';
    </script>";
    exit();
}
...
// Continue with rest of psg_details.php code...
?>