<?php 
    ...
    $uname = $_SESSION["uname"];
        ...
        // Add this after your existing validation code
        if(isset($_POST['confirm_booking'])) {
            try {
                ...
                foreach($selected_seats as $seat_no) {
                    $stmt = $conn->prepare("INSERT INTO seat_bookings 
                        (booking_id, train_no, seat_no, class, user_id, booking_date, journey_date) 
                        VALUES (?, ?, ?, ?, ?, NOW(), ?)");
                    ...
                    switch(strtoupper($class)) {
                        case 'AC':
                            $column_name = 'seats_ac_available';
                            break;
                        case 'SL':
                            $column_name = 'seats_sleeper_available';
                            break;
                        case 'GENERAL':
                            $column_name = 'seats_general_available';
                            break;
                    }

                    if($column_name) {
                        $update_sql = "UPDATE train SET $column_name = $column_name - 1 
                                     WHERE train_no = ?";
